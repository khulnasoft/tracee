...
func NewDefaultProbeGroup(module *bpf.Module, netEnabled bool, kSyms *helpers.KernelSymbolTable) (*ProbeGroup, error) {
	...
	allProbes := map[Handle]Probe{
		...
		SyscallTableCheck:          NewUprobe("syscall_table_check", "uprobe_syscall_table_check", binaryPath, "github.com/khulnasoft/tracee/pkg/ebpf.(*Tracee).triggerSyscallTableIntegrityCheckCall"),
		HiddenKernelModuleSeeker:   NewUprobe("hidden_kernel_module", "uprobe_lkm_seeker", binaryPath, "github.com/khulnasoft/tracee/pkg/ebpf.(*Tracee).triggerKernelModuleSeeker"),
		HiddenKernelModuleVerifier: NewUprobe("hidden_kernel_module", "uprobe_lkm_seeker_submitter", binaryPath, "github.com/khulnasoft/tracee/pkg/ebpf.(*Tracee).triggerKernelModuleSubmitter"),
		PrintNetSeqOps:             NewUprobe("print_net_seq_ops", "uprobe_seq_ops_trigger", binaryPath, "github.com/khulnasoft/tracee/pkg/ebpf.(*Tracee).triggerSeqOpsIntegrityCheckCall"),
		PrintMemDump:               NewUprobe("print_mem_dump", "uprobe_mem_dump_trigger", binaryPath, "github.com/khulnasoft/tracee/pkg/ebpf.(*Tracee).triggerMemDumpCall"),
